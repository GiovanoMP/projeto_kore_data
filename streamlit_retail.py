...
clientes_file = st.sidebar.file_uploader("Upload clientes.csv", type=["csv"])
itens_fatura_file = st.sidebar.file_uploader("Upload itens_fatura.csv", type=["csv"])
produtos_file = st.sidebar.file_uploader("Upload produtos.csv", type=["csv"])
segmentacao_file = st.sidebar.file_uploader("Upload df_treinamento_reduzido.csv", type=["csv"])

if clientes_file and itens_fatura_file and produtos_file and segmentacao_file:
    # Carregar os dataframes
    clientes = load_csv(clientes_file)
    itens_fatura = load_csv(itens_fatura_file)
    produtos = load_csv(produtos_file)
    segmentacao = load_csv(segmentacao_file)
    ...
    st.write("Por favor, faça o upload de todos os arquivos CSV.")
